...
<title>修改帖子-${applicationScope.bbsinfo.name}</title>
     ...
     <div class="z"> 
      ...
      <em>›</em> 
     	<a href="${path}/${bankuai.fid}.html">${bankuai.fname}</a>
      	<em>›</em> 
     	<a href="${path}/${bankuai.yuming}/1_0_0.html">${bankuai.name}</a>
 ...
 <div id="wp" class="wp"> 
   <div id="f_pst" class="bm"> 
    <div class="bm_c"> 
     <form method="post" id="form" action="${path}/tz/update.do"> 
      <div id="fastpostreturn" style="margin:-5px 0 5px"></div> 
      <div class="pbt cl"> 
       <div class="ftid"> 
        <select name="zhutiid"  id="zhutiid"> 
         <c:forEach items="${zhutilist}" var="zhuti">
	         <option value="${zhuti.id}" <c:if test="${tiezi.zhutiid==zhuti.id}">selected="selected"</c:if>  >${zhuti.name}</option>
         </c:forEach>
       </select>
       </div> 
       <input type="text" id="tzname" name="name" class="px" maxlength="80"  tabindex="11" style="width: 560px;" onchange="sysz();" onkeydown="sysz();" onkeyup="sysz();" value="${tiezi.name }"/> 
       <input type="hidden" id="editorhtml" name="contenthtml"/>
       <input type="hidden" id="editortxt" name="contenttxt"/>
       <input type="hidden" id="id" name="id" value="${tiezi.id}"/>
      ...
      <div class="cl"> 
       ...
       <div class="hasfsl" id="fastposteditor"> 
        <div class="tedt"> 
			<script id="myEditor" type="text/plain" style="width:1000px;height:240px;">${tiezi.contenthtml}</script>
        </div> 
       </div> 
      </div> 
